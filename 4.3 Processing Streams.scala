...
assert(df.columns.sameElements(Array("device", "ecommerce", "event_name", "event_previous_timestamp", "event_timestamp", "geo", "items", "traffic_source", "user_first_touch_timestamp", "user_id")) )
...
// TODO
spark.conf.set("spark.sql.shuffle.partition", sc.defaultParallelism)
...
assert(trafficDF.schema.mkString("") == "StructField(traffic_source,StringType,true)StructField(active_users,LongType,false)")
...
val trafficQuery = trafficDF.writeStream
  .queryName("active_users_by_traffic_s")
  .format("memory")
  .outputMode("complete")
...
untilStreamIsReady("active_users_by_traffic_s")
assert(trafficQuery.isActive)
assert(trafficQuery.name.contains("active_users_by_traffic"))
...
display(spark.sql("SELECT * FROM active_users_by_traffic_s"))
